...
# Constants
WAITING_TIME = 4000
ROWS_PER_LOAD = 700  # The number of rows per load
...
def processExcelFile(file_name):
    ...
    for ar in sa:
        with concurrent.futures.ThreadPoolExecutor() as executor:
            futures = {executor.submit(getStockRowData, sym): sym for sym in ar['Symbol']}
...
def get_array_split(list):
    val = int(len(list) / ROWS_PER_LOAD) + 1
    sp = np.array_split(list, val)
    return sp
